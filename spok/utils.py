...
def none_iterable(*args):
    ...
    return all(
        [
            ...
            and not isinstance(arg, np.ndarray)
...
def reshape_to_original_shape(a2d, old_shape):
    if a2d.shape[0] == 1:
        ...
        lst_arrays = np.asarray(
            [
                a2d.reshape(old_shape[-1], np.prod(old_shape[:-1])).T[i :: old_shape[0]]
...
def regular_grid_interpolation(x, y, qty, new_x, new_y, **kwargs):
    ...
    interp = LinearNDInterpolator(xy, qty_2d[:, 0])
...
def nan_gaussian_filter(arr, sigma, mode="nearest"):
    ...
    loss[nan_msk] = 1
    loss = gaussian_filter(loss, sigma=sigma, mode=mode, cval=1)

    gauss = arr / (1 - loss)
    gauss[nan_msk] = 0
    gauss = gaussian_filter(gauss, sigma=sigma, mode=mode, cval=0)
